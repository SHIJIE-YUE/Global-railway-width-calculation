...
os.environ['GDAL_DATA']=r'D:\Anaconda3\Library\share'
    ...
    def download(self,url):
        ...
        while(err<20):
            try:
                ...
                # header = ur.Request(url,headers=HEADER)
                ...
                req.add_header('User-Agent', random.choice(HEADERS))  #换用随机的请求头
...
# ---------------------------------------------------------
MAP_URLS = {
    "Google": "http://gac-geo.googlecnapps.cn/maps/vt?lyrs={style}&x={x}&y={y}&z={z}",
...
def main(left, top, right, bottom, zoom, filePath, style='s', server="Google China"):
    ...
    #     cv2.imshow('1'.result[i])
    #     cv2.imwrite('D:\\map\\result'+str(i)+'.png',result[i])
...
if __name__ == '__main__':
    ...
    data_f=np.loadtxt('C:/Users/Shijie Yue/Desktop/railway generation/location_ms.txt',encoding='latin1',dtype=bytes).astype(bytes)
    delta_data=np.loadtxt('F:/delta_data.txt')
    delta_data_new=delta_data.astype(np.int64)
    delta_data_new=delta_data_new.tolist()
    data=[]
    l=data_f.size
    for i in range(0,l):
        b=data_f[i]    
    ...
    no_work=[]
    for i in delta_data_new:
        i=int(i)
        i=i-1
        lon=data_lon[i]
        lat=data_lat[i]
        pic_west=lon- delta_pic
        pic_north=lat+ delta_pic
        pic_east=lon+ delta_pic
        pic_south=lat- delta_pic
        m=20
        while m>10:
            try:
                path="F:\\map_supply"
                f_path=path+"\\" + str(i)+ "_"+str(m)+".tif"
                old_f_path=path+"\\" + str(i)+".tif"
                if os.path.isfile(old_f_path):
                    os.remove(old_f_path)
                # filepath='r'+f_path
                result=main(pic_west,pic_north,pic_east,pic_south,m,f_path , server="Google")
                for i, data in enumerate(result):
                    picio = io.BytesIO(data)
                    small_pic = pil.open(picio)
                    small_pic.save('D:\\map\\result'+str(i)+'.jpg')
